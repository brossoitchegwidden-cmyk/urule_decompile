...
(
    ...
    CREATE_DATE_  TIMESTAMP    default current  timestamp   not null,     -- 新建日期  
    UPDATE_DATE_  TIMESTAMP    default current  timestamp   not null,     -- 更新日期 
...
create table URULE_PACKET_FILE(
	ID_  	 	INT     PRIMARY KEY	    not null,               								-- 主键
	PACKET_ID_ 		INT           			   not null, 						-- 所属知识包 
	FILE_ID_ 		INT           			   not null, 					-- 关联到的文件ID 
	PROJECT_ID_ 	INT							not	null, 					-- 项目编号
...
create table URULE_DEPLOYED_PACKET(
	...
	PROJECT_ID_ 	INT							not	null, -- 项目编号
	DESC_ 		VARCHAR(255)              not null,                             -- 描述
	APPLY_ID_ 		INT           		 not null, 				-- 发布此知识包的审批流ID
...
create table URULE_DEPLOYED_PACKET_FILE(
	ID_  	 	INT    PRIMARY KEY	    not null,               						-- 主键
	DEPLOYED_PACKET_ID_ 		INT          not null, 					-- 所属发布知识包ID
	PROJECT_ID_ 	INT							not	null, -- 项目编号
	FILE_ID_ 		INT          not null, 							-- 原文件ID
...
create table URULE_PACKET_APPLY(
	...
	CREATE_USER_	VARCHAR(255),												-- 创建人
    CREATE_DATE_         TIMESTAMP     default current  timestamp  not null,      -- 新建日期
   	UPDATE_DATE_         TIMESTAMP     default current  timestamp  not null      -- 新建日期
);
...
(
    ...
    COUNT_SQL_ 		    CLOB  null,												-- 总记录数sql
	VALIDATOR_DATA_ 	CLOB  null,												-- 数据校验器
    DESC_				VARCHAR(255)                        null,		        -- 描述
    CREATE_USER_		VARCHAR(255),													-- 文件创建人 
    UPDATE_USER_		VARCHAR(255),													-- 文件更新人 
    CREATE_DATE_        TIMESTAMP     default current  timestamp not null,      			-- 新建日期
...
(
    ...
    SRC_PROPERTY_		VARCHAR(255)							not null,		-- 原属性名称,如果DATA_TYPE_值是Provider则该值是provider的id
...
(
    ...
    EXPIR_DATE_          TIMESTAMP     default current  timestamp  	not null,  -- 密码找回过期时间
    CREATE_DATE_         TIMESTAMP     default current  timestamp not null,   -- 新建日期
    UPDATE_DATE_         TIMESTAMP     default current  timestamp not null   -- 更新日期
);
...
(
    ...
    CREATE_DATE_    TIMESTAMP    default current  timestamp  not null      -- 加入日期
);
...
(
    ...
    EXPIR_DATE_     TIMESTAMP     default current  timestamp  not null,  -- 过期时间
    CREATE_USER_	VARCHAR(255),									-- 创建人 
    CREATE_DATE_    TIMESTAMP     default current  timestamp  not null  -- 创建日期
);
...
(
    ...
    CREATE_DATE_    TIMESTAMP    default current  timestamp   not null  -- 创建时间
);
...
(
    ...
    START_TIME_     TIMESTAMP    default current  timestamp   not null,  -- 调用开始时间
    END_TIME_     	TIMESTAMP    default current  timestamp   not null,  -- 调用结束时间
    TIME_			INT              	null, -- 耗时
    GROUP_ID_ 		VARCHAR(50)					not null, -- 团队编号
    GROUP_NAME_ 	VARCHAR(255)					null, -- 团队名称
   	PROJECT_ID_ 	INT							not null, -- 项目编号
...
(
    ...
    START_TIME_     TIMESTAMP      default current  timestamp  not null,  -- 调用开始时间
    END_TIME_     	TIMESTAMP                       null,  -- 调用结束时间
    TIME_			INT 			                 	null, -- 耗时
