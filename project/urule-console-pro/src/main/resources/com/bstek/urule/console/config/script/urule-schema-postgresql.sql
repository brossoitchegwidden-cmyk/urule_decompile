...
(
    ...
    PACKAGE_ID_ 	 BIGINT           not null,      			-- 所属目录
    PROJECT_ID_ 	 BIGINT           not null,					-- 所属项目  
    CONTENT_      	TEXT                              null,     			-- 内容  
    ...
    DELETED_		BOOLEAN		  	default FALSE not null 					-- 删除标记  
);
...
(
    ...
    PARENT_ID_ 		BIGINT         not null,                 -- 所属目录
    PROJECT_ID_ 		BIGINT        not null,					-- 所属项目  
    CREATE_USER_	VARCHAR(255),			 							-- 文件创建人  
    UPDATE_USER_	VARCHAR(255),			 							-- 文件更新人  
    CREATE_DATE_  TIMESTAMP    DEFAULT CURRENT_TIMESTAMP   not null,     -- 新建日期  
...
create table URULE_PACKET_PACKAGE(
	ID_  			BIGINT PRIMARY KEY             not null,               				-- 主键
	PACKET_ID_ 		BIGINT            			   not null, 					-- 所属知识包 
	PROJECT_ID_ 	BIGINT							not	null, 					-- 项目编号
...
create table URULE_PACKET_FILE(
	ID_  	 	BIGINT     PRIMARY KEY	    not null,               								-- 主键
	PACKET_ID_ 		BIGINT           			   not null, 						-- 所属知识包 
	FILE_ID_ 		BIGINT           			   not null, 					-- 关联到的文件ID 
	PROJECT_ID_ 	BIGINT							not	null, 					-- 项目编号
...
create table URULE_DEPLOYED_PACKET(
	...
	PROJECT_ID_ 	BIGINT							not	null, -- 项目编号
	DESC_ 		VARCHAR(255)              not null,                             -- 描述
	APPLY_ID_ 		BIGINT           		 not null, 				-- 发布此知识包的审批流ID
...
create table URULE_DEPLOYED_PACKET_FILE(
	ID_  	 	BIGINT    PRIMARY KEY	    not null,               						-- 主键
	DEPLOYED_PACKET_ID_ 		BIGINT          not null, 					-- 所属发布知识包ID
	PROJECT_ID_ 	BIGINT							not	null, -- 项目编号
	FILE_ID_ 		BIGINT          not null, 							-- 原文件ID
...
create table URULE_PACKET_APPLY(
	...
	CREATE_USER_	VARCHAR(255),												-- 创建人
    CREATE_DATE_         TIMESTAMP     DEFAULT CURRENT_TIMESTAMP  not null,      -- 新建日期
   	UPDATE_DATE_         TIMESTAMP     DEFAULT CURRENT_TIMESTAMP  not null      -- 新建日期
);
...
(
    ...
    DB_INITIAL_SIZE_    BIGINT 							        null,		-- 初始化连接数
    DB_MAX_TOTAL_       BIGINT 							        null,		-- 最大活动连接数
    DB_MAX_IDLE_        BIGINT 							        null,		-- 最大空闲连接数
    DB_MIN_IDLE_        BIGINT 							        null,		-- 最小空闲连接数
...
(
	ID_  	 			BIGINT     PRIMARY KEY	    	    not null,				-- 主键
   	PROJECT_ID_ 	    BIGINT 							    not	null,               -- 项目编号
    NAME_				VARCHAR(255)					not null,				-- 名称
    ENABLE_ 		    BOOLEAN default FALSE	not null, 						-- 是否启用
    SKIP_LIMIT_         BIGINT							        null,               -- 异常数量限定
    ASYNC_              BOOLEAN default TRUE	        not null, 				-- 是否启用异步处理
    CALLBACK_URL_		VARCHAR(255)					    null,				-- 异步回调URL
    STATUS_             VARCHAR(20)                 	    null,               -- 状态: 状态: completed:成功, failed:失败, started:执行中,  none:无
    LISTENER_   	    VARCHAR(255)					    null,				-- 批处理监测类
    THREAD_MULTI_       BOOLEAN default TRUE	        not null, 				-- 是否启用多线程处理
    THREAD_SIZE_        BIGINT 							        null,               -- 开启线程数
    THREAD_DATA_SIZE_   BIGINT 							        null,               -- 单个线程处理数据量
    PROVIDER_ID_ 	    BIGINT 							        null,               -- 数据查询方案编号
    RESOLVER_ID_ 	    BIGINT 							        null,               -- 数据保存方案编号
...
(
    ...
    SUPPORT_PAGING_     BOOLEAN DEFAULT FALSE            not null,               -- 是否支持分页查询,多线程情况下默认使用分页查询
...
(
	ID_  	 			BIGINT     PRIMARY KEY	    	    not null,				-- 主键
   	PROJECT_ID_ 	    BIGINT 							    not	null,               -- 项目编号
    BATCH_ID_			BIGINT                             	not null,				-- 所属方案ID
    PROVIDER_ID_ 	    BIGINT 							    not	null,               -- 查询方案编号
    ...
    DATA_PROVIDER_ID_ 	BIGINT							        null,               -- DATA_TYPE_类型为Object或List时对应查询方案编号
    CREATE_USER_		VARCHAR(255),													-- 文件创建人 
    UPDATE_USER_		VARCHAR(255),													-- 文件更新人 
    CREATE_DATE_        TIMESTAMP     default CURRENT_TIMESTAMP not null,      			-- 新建日期
    UPDATE_DATE_        TIMESTAMP     default CURRENT_TIMESTAMP not null      			-- 更新日期
...
(
    ...
    BATCH_ID_			BIGINT                             	not null,				-- 所属方案ID
   	PROJECT_ID_ 	    BIGINT 							    not	null,               -- 项目编号
    RESOLVER_ID_ 	    BIGINT 							    not	null,               -- 保存方案编号
    NAME_				VARCHAR(255)					not null,		        -- 名称
...
(
    ...
    BATCH_ID_			BIGINT                             	not null,				-- 所属方案ID
    RESOLVER_ID_ 	    BIGINT 							    not	null,               -- 保存方案编号
    ITEM_ID_ 	        BIGINT 							    not	null,               -- 保存方案明细项编号
    SRC_PROPERTY_	    VARCHAR(255)							not null,		-- 原属性名称
    DATA_TYPE_			VARCHAR(255)							not null,		-- 数据类型
    KEY_                BOOLEAN		  	        default FALSE not null,       -- 是否主键
    DEST_PROPERTY_		VARCHAR(255)                            not null,		-- 目标属性名称
    CREATE_USER_		VARCHAR(255),													-- 文件创建人 
    UPDATE_USER_		VARCHAR(255),													-- 文件更新人 
    CREATE_DATE_        TIMESTAMP     default CURRENT_TIMESTAMP not null,      			-- 新建日期
...
(
    ...
    EXPIR_DATE_          TIMESTAMP     DEFAULT CURRENT_TIMESTAMP  	not null,  -- 密码找回过期时间
    CREATE_DATE_         TIMESTAMP     DEFAULT CURRENT_TIMESTAMP not null,   -- 新建日期
    UPDATE_DATE_         TIMESTAMP     DEFAULT CURRENT_TIMESTAMP not null   -- 更新日期
);
...
(
    ...
    USER_ID_ 		VARCHAR(50)	not null, -- 用户
    USER_NAME_ 		VARCHAR(255)   not null, -- 用户名
    PROJECT_ID_ 	BIGINT			not null, -- 项目
    CREATE_DATE_    TIMESTAMP    DEFAULT CURRENT_TIMESTAMP  not null      -- 加入日期
);
...
(
    ...
    AUTH_ 			INT           default 1  not null -- 授权信息
);
...
(
    ...
    CREATE_DATE_    TIMESTAMP    DEFAULT CURRENT_TIMESTAMP   not null  -- 创建时间
);
...
(
    ...
    IP_				VARCHAR(50)                 	null, -- 登录地址
    USER_AGENT_		VARCHAR(255) 	                null, -- 用户代理
    KNOWLEDGE_ID_	BIGINT		                 	null, -- 知识包ID
    KNOWLEDGE_NAME_	VARCHAR(100)                 	null, -- 知识包Name
    VERSION_		VARCHAR(30)                 	null, -- 版本号
    IN_PARAMS_      TEXT               not null, -- 输入参数
    OUT_PARAMS_     TEXT               not null, -- 输出参数
    LOGS_		    TEXT               not null, -- 执行日志
    START_TIME_     TIMESTAMP    DEFAULT CURRENT_TIMESTAMP   not null,  -- 调用开始时间
    END_TIME_     	TIMESTAMP    DEFAULT CURRENT_TIMESTAMP   not null,  -- 调用结束时间
    TIME_			BIGINT              	null, -- 耗时
    GROUP_ID_ 		VARCHAR(50)					not null, -- 团队编号
    GROUP_NAME_ 	VARCHAR(255)					null, -- 团队名称
   	PROJECT_ID_ 	BIGINT						not null, -- 项目编号
    PROJECT_NAME_ 	VARCHAR(255)					null, -- 项目名称
