...
public class UserQueryImpl implements UserQuery {
   ...
   public Page users(int pageIndex, int pageSize, long projectId) {
      String pageSql = "SELECT USER_ID_, USER_NAME_, CREATE_DATE_ FROM URULE_PROJECT_USER  WHERE PROJECT_ID_=?";
      ...
      try {
         ...
         pageSql = "select count(*) from URULE_PROJECT_USER where PROJECT_ID_=?";
         if (stringBuilder.length() > 0) {
            pageSql = pageSql + " and" + stringBuilder.toString();
         }
   ...
   private StringBuilder buildWhereClause() {
      ...
      if (this.id != null) {
         ...
         stringBuilder.append(" URULE_PROJECT_USER.USER_ID_ like ?");
      ...
      if (this.name != null) {
         ...
         stringBuilder.append(" URULE_PROJECT_USER.USER_NAME_ like ?");
      ...
      if (this.idnameLike != null) {
         ...
         stringBuilder.append(" (URULE_PROJECT_USER.USER_ID_ like ? or URULE_PROJECT_USER.USER_NAME_ like ?) ");
   ...
   public Page roleUsers(int pageIndex, int pageSize, long projectId, long roleId) {
      String pageSql = "SELECT URULE_PROJECT_USER.USER_ID_, URULE_PROJECT_USER.USER_NAME_, URULE_PROJECT_USER.CREATE_DATE_ FROM URULE_PROJECT_USER  LEFT JOIN URULE_PROJECT_USER_ROLE on URULE_PROJECT_USER_ROLE.USER_ID_=URULE_PROJECT_USER.USER_ID_ WHERE URULE_PROJECT_USER_ROLE.ROLE_ID_=? and URULE_PROJECT_USER.PROJECT_ID_=?";
      this.queryParameters.clear();
      ...
      try {
         ...
         JdbcUtils.closeResultSet(resultSet);
         JdbcUtils.closeStatement(preparedStatement);
         pageSql = "select count(*) FROM URULE_PROJECT_USER  LEFT JOIN URULE_PROJECT_USER_ROLE on URULE_PROJECT_USER_ROLE.USER_ID_=URULE_PROJECT_USER.USER_ID_ WHERE URULE_PROJECT_USER_ROLE.ROLE_ID_=? and URULE_PROJECT_USER.PROJECT_ID_=?";
         if (stringBuilder.length() > 0) {
            pageSql = pageSql + " and" + stringBuilder.toString();
         }
